...
export default function Dashboard() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', flexDirection: 'column', gap: 16 }}>
      ...
      <span style={{ color: '#94A3B8', fontSize: 13 }}>Chargement du tableau de bord…</span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* Status bar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 16px', background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 10, fontSize: 12, flexWrap: 'wrap' }}>
        {[
          { label: 'API', ok: true },
        ...
        <span style={{ marginLeft: 'auto', color: '#94A3B8' }}>SLA 99.8% ce mois</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 18 }}>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>7 derniers jours · XAF</div>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              {['7J', '30J', '90J'].map((l, i) => (
                <button key={l} style={{ padding: '4px 10px', border: '1px solid #D1D5DB', borderRadius: 20, background: i === 0 ? '#F8FAFC' : 'transparent', color: i === 0 ? '#0F172A' : '#94A3B8', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>{l}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 12 }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #E5E7EB' }}>
            ...
            <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>Répartition ce mois</div>
          </div>
          <div style={{ padding: '16px 20px' }}>
            ...
            <div style={{ marginTop: 18, paddingTop: 14, borderTop: '1px solid #EEF0F2' }}>
              <div style={{ fontSize: 11, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 4 }}>Volume total</div>
              <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: '-.5px' }}>{stats.totalCA.toLocaleString('fr-FR')} <span style={{ fontSize: 13, fontWeight: 400, color: '#475569' }}>XAF</span></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 12 }}>
        {[
          ...
          { label: 'Zones couvertes', value: '4', color: '#0F172A' },
          ...
          <div key={i} style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 12, padding: '14px', textAlign: 'center' }}>
...
function MiniChart({ data }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 160 }}>
      ...
      {pts.map((p, i) => (
        <g key={i}>
          <circle cx={p.x} cy={p.y} r={i === pts.length - 1 ? 4 : 3} fill="#00e5a0" stroke="#F8FAFC" strokeWidth="2" />
          <text x={p.x} y={H - 6} fill="#94A3B8" fontSize="10" textAnchor="middle" fontFamily="system-ui">{p.jour}</text>
        </g>
      ))}
    </svg>
  );
