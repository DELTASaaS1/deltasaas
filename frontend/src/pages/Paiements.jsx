...
export default function Paiements() {
  ...
  const COLS = [
    { key: 'id', label: 'Référence', render: v => <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#475569' }}>{v}</span> },
    ...
    { key: 'reference', label: 'Réf. opérateur', render: v => <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#94A3B8' }}>{v}</span> },
    ...
    { key: 'date', label: 'Date', render: v => <span style={{ fontSize: 12, color: '#94A3B8' }}>{new Date(v).toLocaleString('fr-FR')}</span> },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {[
          ...
          return (
            <div key={i} style={{ background: '#FFFFFF', border: `1px solid ${color}22`, borderRadius: 12, padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <span style={{ fontSize: 20 }}>{p.icon}</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{p.mode}</div>
                  <div style={{ fontSize: 11, color: '#94A3B8' }}>{p.pct}% des transactions</div>
                </div>
                <span style={{ marginLeft: 'auto', fontSize: 16, fontWeight: 700, color }}>{p.pct}%</span>
              </div>
              <div style={{ height: 5, background: '#E5E7EB', borderRadius: 4, marginBottom: 8 }}>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid #EEF0F2', background: '#F8FAFC', display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#94A3B8' }}>
