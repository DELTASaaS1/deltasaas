  ...
  const COLS = [
    { key: 'nom', label: 'Produit', render: (v, row) => (
      <div>
        ...
        <div style={{ fontSize: 11, color: '#555e78', fontFamily: 'monospace', marginTop: 2 }}>{row.sku}</div>
      </div>
    )},
    { key: 'categorie', label: 'Catégorie', render: v => <span style={{ fontSize: 11, padding: '2px 7px', background: '#181c26', border: '1px solid #ffffff12', borderRadius: 6, color: '#8b92a8' }}>{v}</span> },
    ...
    { key: 'stock', label: 'Stock', render: (v, row) => (
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ flex: 1, height: 4, background: '#1e2330', borderRadius: 4, minWidth: 60 }}>
        ...
        <span style={{ fontFamily: 'monospace', fontWeight: 600, minWidth: 24, textAlign: 'right', color: v <= row.seuil_alerte ? '#ff5a5a' : '#e8ecf4' }}>{v}</span>
      </div>
    )},
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        <StatCard label="Produits actifs" value={produits.length} change="8 catégories" color="#e8ecf4" />
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <button
          ...
          style={{ padding: '7px 13px', border: `1px solid ${filtreAlerte ? '#ff5a5a44' : '#ffffff1e'}`, borderRadius: 8, background: filtreAlerte ? '#ff5a5a15' : 'transparent', color: filtreAlerte ? '#ff5a5a' : '#8b92a8', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}
        >
      ...
      {/* Tableau produits */}
      <div>
        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 10, color: '#8b92a8' }}>Inventaire produits</div>
        <Table columns={COLS} data={produits} loading={loading} onRowClick={setSelected} />
      </div>

      {/* Historique mouvements */}
      <div>
        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 10, color: '#8b92a8' }}>Historique des mouvements</div>
      ...
      <Modal open={modalAjust} onClose={() => setModalAjust(false)} title={`Ajuster le stock — ${selected?.nom}`}>
        <div style={{ padding: '12px', background: '#181c26', borderRadius: 8, marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 12, color: '#8b92a8' }}>Stock actuel</span>
        ...
        {ajustQte && (
          <div style={{ padding: '10px 12px', background: '#181c26', borderRadius: 8, marginBottom: 14, fontSize: 13 }}>
