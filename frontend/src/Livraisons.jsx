...
export default function Livraisons() {
  ...
  const COLS = [
    { key: 'id', label: 'ID livraison', render: v => <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#8b92a8' }}>{v}</span> },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        <StatCard label="Total livraisons" value={livraisons.length} color="#e8ecf4" />
      ...
      {/* Livreurs cards */}
      <div>
        <div style={{ fontSize: 13, fontWeight: 500, color: '#8b92a8', marginBottom: 10 }}>Livreurs</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
          {livreurs.map(d => (
            <div key={d.id} style={{ background: '#12151c', border: `1px solid ${d.actif ? '#00e5a022' : '#ffffff12'}`, borderRadius: 12, padding: 16, display: 'flex', gap: 12, alignItems: 'center' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: d.actif ? '#00e5a020' : '#ffffff0a', color: d.actif ? '#00e5a0' : '#555e78', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {d.nom.split(' ').map(n => n[0]).join('').slice(0, 2)}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 500, fontSize: 13 }}>{d.nom}</div>
                <div style={{ fontSize: 11, color: '#555e78', marginTop: 2 }}>{d.zone}</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 6, alignItems: 'center' }}>
                  <span style={{ fontSize: 11, padding: '1px 6px', borderRadius: 10, background: d.actif ? '#00e5a020' : '#ffffff0a', color: d.actif ? '#00e5a0' : '#555e78' }}>
                    {d.actif ? '● Actif' : '○ Inactif'}
                  </span>
                  <span style={{ fontSize: 11, color: '#555e78' }}>{d.livraisons_jour} livraisons/j</span>
        ...
        <div style={{ background: '#12151c', border: '1px solid #ffffff1e', borderRadius: 12, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderBottom: '1px solid #ffffff12' }}>
            {[
              ...
              <div key={ci} style={{ padding: '16px 20px', borderRight: ci < 2 ? '1px solid #ffffff12' : 'none' }}>
                <div style={{ fontSize: 10, color: '#555e78', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 10 }}>{col.title}</div>
                {col.rows.map(r => (
                  <div key={r.l} style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 10, color: '#555e78', marginBottom: 2 }}>{r.l}</div>
                    <div style={{ fontSize: r.big ? 22 : 13, fontWeight: r.big ? 600 : 500, fontFamily: r.mono || r.big ? 'monospace' : 'inherit', color: r.big ? '#00e5a0' : '#e8ecf4', letterSpacing: r.big ? '.15em' : 'normal' }}>{r.v || '—'}</div>
                  </div>
                ))}
              </div>
            ))}
      ...
      <Modal open={modalCode} onClose={() => { setModalCode(false); setCode(''); setErrCode(''); }} title="Confirmer la livraison">
        <div style={{ padding: '12px', background: '#181c26', borderRadius: 8, marginBottom: 16 }}>
          <div style={{ fontSize: 12, color: '#8b92a8', marginBottom: 4 }}>Le livreur demande au client son code à 4 chiffres</div>
